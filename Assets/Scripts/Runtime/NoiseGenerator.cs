...
using System.Linq;
using UnityEngine;
using FuncType = GhostOfTsushima.Runtime.NoiseOctave.FunctionType;
...
namespace GhostOfTsushima.Runtime
{
    public class NoiseGenerator
    {
        ...
        public static HashSet<FuncType> AllTypes
        {
            get => _types ??= new HashSet<FuncType>
        ...
        public float CalculateNoiseByOctaves(Vector2 position, List<NoiseOctave> octaves)
        {
            ...
            {
                perlinValue = Mathf.PerlinNoise(position.x * frequency * _scaleValue,
                    position.y * frequency * _scaleValue);
        ...
        public IEnumerable<NoiseOctave> GenerateRandomOctaves(int count, IEnumerable<FuncType> includedFunctions = null)
        {
            ...
            return Enumerable.Range(0, count).Select(x =>
            {
                FuncType functionType =
                    includedFunctions is null ? includedFunctions.GetRandom() : AllTypes.GetRandom();
